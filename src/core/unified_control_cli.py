...
from src.core.unified_pipeline_controller import UnifiedPipelineController, PipelineStatus
...
class UnifiedControlCLI:
    ...
    def start_controller(self, background: bool = False):
        ...
        if self.controller and self.running:
            print("âŒ Controller is already running")
            return
        
        print("ðŸš€ Starting Tunisia Intelligence Unified Pipeline Controller...")
        ...
        if background:
            ...
            self.running = True
            print("âœ… Controller started in background")
        else:
            try:
    ...
    def stop_controller(self):
        ...
        if not self.controller or not self.running:
            print("âŒ Controller is not running")
            return
        
        print("ðŸ›‘ Stopping controller...")
    ...
    def resume_controller(self):
        ...
        print("â–¶ï¸ Controller resumed")
    
    def show_status(self):
        """Show current controller and pipeline status."""
        control_settings = get_unified_control()
        
        print("ðŸŽ›ï¸ Tunisia Intelligence Unified Control Status")
        print("=" * 60)
        ...
        print(f"Debug Mode: {'ðŸ›' if control_settings.debug else 'âœ…'}")
        ...
        if self.controller and self.running:
            ...
            print(f"   Running: {'âœ…' if controller_status['running'] else 'âŒ'}")
            print(f"   Paused: {'â¸ï¸' if controller_status['paused'] else 'â–¶ï¸'}")
            ...
            print(f"\nðŸŽ¯ Controller Status: âŒ Not Running")
    ...
    def run_single_pipeline(self, pipeline_name: str):
        ...
        if pipeline_name not in valid_pipelines:
            print(f"âŒ Invalid pipeline: {pipeline_name}")
            print(f"Valid pipelines: {', '.join(valid_pipelines)}")
            return
        
        print(f"ðŸŽ¯ Running {pipeline_name} pipeline...")
        ...
        try:
            ...
            print(f"âŒ Pipeline execution failed: {e}")
    ...
    def show_configuration(self, section: Optional[str] = None):
        ...
        print("âš™ï¸ Tunisia Intelligence Configuration")
        print("=" * 50)
        ...
        if section:
            if section in config_dict:
                ...
                print(f"âŒ Configuration section '{section}' not found")
    ...
    def set_configuration(self, parameter_path: str, value: str):
        ...
        print(f"âš™ï¸ Setting {parameter_path} = {value}")
        
        # Parse parameter path (e.g., "rss.batch_size" or "ai_enrichment.model_name")
        path_parts = parameter_path.split('.')
        
        if len(path_parts) != 2:
            print("âŒ Parameter path must be in format: section.parameter")
        ...
        print(f"âœ… Set {env_var_name} = {converted_value}")
        print("ðŸ”„ Reload configuration to apply changes")
    ...
    def show_logs(self, lines: int = 50, follow: bool = False):
        ...
        if not log_file.exists():
            print("âŒ Log file not found")
            return
        
        print(f"ðŸ“‹ Recent Logs (last {lines} lines):")
        print("=" * 60)
        ...
        if follow:
            ...
            try:
                ...
                print(f"âŒ Error reading log file: {e}")
    ...
    def _follow_logs(self, log_file: Path):
        ...
        try:
            with open(log_file, 'r', encoding='utf-8') as f:
                ...
                print("ðŸ“¡ Following logs (Ctrl+C to stop)...")
            ...
            print(f"âŒ Error following logs: {e}")
    
    def show_metrics(self):
        """Show pipeline metrics and statistics."""
        if not self.controller or not self.running:
            print("âŒ Controller is not running - showing configuration only")
        ...
        if pipeline_metrics:
            for pipeline_name, metrics in pipeline_metrics.items():
                print(f"\nðŸ”§ {pipeline_name.replace('_', ' ').title()}:")
    ...
    def interactive_mode(self):
        ...
        print("ðŸŽ›ï¸ Tunisia Intelligence Unified Control - Interactive Mode")
    ...
    try:
        if args.command == 'start':
            ...
            if args.config_action == 'show':
                ...
                print("âŒ Config action required: show, set, or reload")
