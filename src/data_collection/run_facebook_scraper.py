...
from src.data_collection.facebook_loader import UltraMinimalFacebookLoader
...
def _setup_facebook_token_automatically(secret_manager):
    ...
    try:
        ...
        if success:
            logger.info("âœ… Facebook app token stored successfully")
            
            # Also set API version
            secret_manager.set_secret("FACEBOOK_API_VERSION", "v18.0")
            logger.info("âœ… Facebook API version set to v18.0")
            
            # Set environment variable to use file backend
            import os
            os.environ["SECRETS_BACKEND"] = "file"
            logger.info("âœ… Secret backend configured for file storage")
        else:
            logger.error("âŒ Failed to store Facebook app token")
...
def main():
    ...
    if result.get('status') == 'completed':
        ...
        if efficiency <= 4.0:
            ...
            print("Efficiency Rating: âŒ NEEDS OPTIMIZATION")
